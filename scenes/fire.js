class Fire extends Phaser.Scene {
  ...
  create() {
    ...
    var lawIcon = this.add.image(42, 762, 'icons_clear', 48).setOrigin(.5).setScale(1.5).setInteractive().setAlpha(1);
    this.nameText = this.add.text(100, 747, 'Fire Data ', { fontFamily: 'PixelFont', fontSize: '30px', color: '#0057AF', align: 'left', backgroundColor: '#A6CAF0' })
    ...
    var fsRad = gameRules.fsRadius - getCovSub(sim.gameData.maintenanceCostsPer[15])
    var fhRad = gameRules.fhRadius - getCovSub(sim.gameData.maintenanceCostsPer[15])
    ...
    this.fundText = this.add.bitmapText(25, 1000, 'topaz', 'Funding Level ' + sim.gameData.maintenanceCostsPer[15] + '%', 40).setOrigin(0, .5).setTint(0xC0D7E2).setInteractive();
    this.psRadText = this.add.bitmapText(25, 1075, 'topaz', 'Station Coverage: ' + fsRad, 40).setOrigin(0, .5).setTint(0xC0D7E2).setInteractive();
    this.phRadText = this.add.bitmapText(25, 1150, 'topaz', 'Headquarters Coverage: ' + fhRad, 40).setOrigin(0, .5).setTint(0xC0D7E2).setInteractive();
    ...
    if (sim.gameData.fireStations.length > 0) {
      ...
      for (var i = 0; i < sim.gameData.fireStations.length; i++) {
        ...
        text += plant.name
        text += ', ' + sim.gameData.fireStations[i][2]
        text += ', ' + 5 + '\n'
    ...
    this.numText = this.add.text(25, 1300, 'Stations: ' + sim.gameData.fireStations.length, { fontFamily: 'PixelFont', fontSize: '30px', color: '#C0D7E2', align: 'left' })
